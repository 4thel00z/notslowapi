...
def test_staticfiles_with_package(test_client_factory: TestClientFactory) -> None:
    app = StaticFiles(packages=["tests.starlette"])
    client = test_client_factory(app)
    response = client.get("/example.txt")
    assert response.status_code == 200
    assert response.text == "123\n"

    app = StaticFiles(packages=[("tests.starlette", "statics")])
    client = test_client_factory(app)
    response = client.get("/example.txt")
    assert response.status_code == 200
    assert response.text == "123\n"
...
def test_staticfiles_unhandled_os_error_returns_500(
    ...
    monkeypatch.setattr("notslowapi.starlette.staticfiles.StaticFiles.lookup_path", mock_timeout)
