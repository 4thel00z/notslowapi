...
def test_deprecated_types(constant: str, msg: str) -> None:
    with pytest.warns(StarletteDeprecationWarning) as record:
        getattr(importlib.import_module("notslowapi.starlette.status"), constant)
        assert len(record) == 1
...
def test_unknown_status() -> None:
    with pytest.raises(
        AttributeError,
        match="module 'notslowapi.starlette.status' has no attribute 'HTTP_999_UNKNOWN_STATUS_CODE'",
    ):
        getattr(importlib.import_module("notslowapi.starlette.status"), "HTTP_999_UNKNOWN_STATUS_CODE")
