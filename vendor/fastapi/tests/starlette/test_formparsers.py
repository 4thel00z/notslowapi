...
from notslowapi.starlette.datastructures import Headers, UploadFile
from notslowapi.starlette.formparsers import FormParser, MultiPartException, MultiPartParser, _user_safe_decode
from notslowapi.starlette.requests import Request
from notslowapi.starlette.responses import JSONResponse
from notslowapi.starlette.routing import Mount
...
def mock_spooled_temporary_file() -> Generator[None]:
    try:
        with mock.patch("notslowapi.starlette.formparsers.SpooledTemporaryFile", ThreadTrackingSpooledTemporaryFile):
            yield
    finally:
...
async def test_multipart_closes_tempfile_on_stream_error() -> None:
    ...
    with (
        mock.patch("notslowapi.starlette.formparsers.SpooledTemporaryFile", return_value=tempfile),
...
def test_multipart_closes_tempfile_on_oserror(
    ...
    async def error_app(scope: Scope, receive: Receive, send: Send) -> None:
        request = Request(scope, receive)
        with mock.patch("notslowapi.starlette.formparsers.SpooledTemporaryFile", FailingSpooledTemporaryFile):
            await request.form()
