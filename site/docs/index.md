...
One core, Apple M3 Pro, Python 3.13, 64 keep-alive connections, median of 3 x 5 s runs. The first column is upstream FastAPI 0.141.1 on uvicorn at the start of the work; the other two are the current master (all 43 changes).

| route | FastAPI 0.141 on uvicorn | notslowapi on uvicorn | notslowapi on granian |
|---|---|---|---|
| raw ASGI app (server floor) | 13.7 µs/req, 73,040 req/s | 13.2 µs/req, 75,943 req/s | 7.9 µs/req, 126,946 req/s |
| plain JSON route | 31.2 µs/req, 32,033 req/s | 15.7 µs/req, 63,757 req/s | 8.1 µs/req, 123,416 req/s |
| int path + str query param | 57.0 µs/req, 17,531 req/s | 17.0 µs/req, 58,977 req/s | 8.8 µs/req, 113,996 req/s |
| pydantic body + response_model | 52.1 µs/req, 19,212 req/s | 18.0 µs/req, 55,692 req/s | 12.4 µs/req, 80,445 req/s |
| 50 routes via include_router | 92.2 µs/req * | 15.2 µs/req, 65,887 req/s | 8.3 µs/req, 120,211 req/s |
| three dependencies (path, header, query via Depends) | not measured | 22.1 µs/req, 45,296 req/s | 11.1 µs/req, 90,259 req/s |

Staying on uvicorn: 2.0x the requests per core on the plain route, 3.4x on typed parameters, 2.9x on a pydantic body. Moving the plain route to granian: 3.9x. The before column is `bench/baseline/results_ladder_v1.json`; both after columns are `results_ladder_v6.json`, the same files the landing page and the README quote.
