...
`myapp:app` is the module path and attribute of your `FastAPI` instance. granian's I/O runs on Rust threads alongside the Python thread, so the framework's Python is the only thing left on the critical path. On the bare ASGI callable the server alone costs 7.9 µs per request under granian against 13.2 µs under uvicorn, and the notslowapi route adds 0.2 µs on top of that under granian against 2.5 µs under uvicorn.

If you stay on uvicorn, `uvicorn[standard]` gives you uvloop and httptools (15.7 µs on the plain route against 53.2 µs on asyncio and h11), and the three `--no-*` flags are worth about 5 percent unless you sit behind a proxy that sets `X-Forwarded-*` headers:
